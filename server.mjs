...
const pool = new pg.Pool({
  connectionString: process.env.DATABASE_URL || "postgresql://localhost:5432/roomsystem",
...
// In-memory room storage for active rooms
...
const sessionMiddleware = session({
  ...
  secret: process.env.SESSION_SECRET || "room-system-secret-key",
  resave: false,
...
async function getOrCreateUser(sessionId, name = null) {
  ...
  result = await pool.query(
    "INSERT INTO users (session_id, name) VALUES ($1, $2) RETURNING id, name",
    [sessionId, name]
  );
